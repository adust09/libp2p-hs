...
import Control.Concurrent.Async (concurrently, concurrently_, replicateConcurrently)
...
spec = do
  describe "Stream ID allocation" $ do
    it "client session allocates odd IDs: 1, 3, 5" $ do
      ...
      ids <- concurrently
        ...
        ( do
            -- Accept all three on the server side
    ...
    it "server session allocates even IDs: 2, 4, 6" $ do
      ...
      ids <- concurrently
        ( do
            -- Accept all three on the client side
  ...
  describe "Stream open/accept" $ do
    it "openStream sends Data frame with SYN flag" $ do
      ...
      -- Verify the SYN frame was queued (check the send channel or the remote side)
  ...
  describe "Data exchange" $ do
    ...
    it "optimistic sending: data piggybacked on SYN frame" $ do
      (client, server) <- mkSessionPair
      -- Open stream and immediately write (before ACK)
  ...
  describe "Stream rejection" $ do
    it "RST response to SYN -> streamWrite returns Reset" $ do
      ...
      Right clientStream <- openStream client
      -- Server accepts then immediately resets
      Right serverStream <- acceptStream server
  ...
  describe "Concurrent streams" $ do
    it "4 concurrent streams on single session" $ do
      (client, server) <- mkSessionPair
      -- Open 4 streams and exchange data on each
      let numStreams = 4
      concurrently_
        ( do
  ...
  describe "Ping" $ do
    ...
    it "Ping echoes exact opaque value in Length field" $ do
      -- Verified internally by the ping implementation:
      -- ping sends a Ping frame with a specific value, and the response
      -- must contain the same value. If it doesn't match, ping returns error.
      (client, server) <- mkSessionPair
      result <- ping client
      result `shouldBe` Right ()

    it "Ping uses StreamID 0" $ do
      -- Ping frames must use StreamID 0 per spec.
      -- Verified by implementation: recvLoop only handles Ping on StreamID 0.
      (client, server) <- mkSessionPair
      result <- ping client
      result `shouldBe` Right ()
